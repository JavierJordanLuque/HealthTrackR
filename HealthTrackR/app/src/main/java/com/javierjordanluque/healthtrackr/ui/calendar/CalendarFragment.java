...
public class CalendarFragment extends Fragment {
        ...
        buttonFilterCalendar.setOnClickListener(view -> {
            ...
            buttonFilter.setOnClickListener(v -> {
                ...
                try {
                    ...
                    for (Treatment treatment : user.getTreatments(requireActivity())) {
                        filteredAppointments.addAll(treatment.filterAppointments(passedAppointmentsFilter, pendingAppointmentsFilter));
                        filteredMedicines.addAll(treatment.getMedicines(requireActivity()));
                    }

                    showHighlightedMedicalAppointments(filteredAppointments);
                    showHighlightedMedications(filteredMedicines);
            ...
            buttonShowAllFilter.setOnClickListener(v -> {
                try {
                    ...
                    List<MedicalAppointment> unfilteredAppointments = new ArrayList<>();
                    List<Medicine> unfilteredMedicines = new ArrayList<>();
                    for (Treatment treatment : user.getTreatments(requireActivity())) {
                        unfilteredAppointments.addAll(treatment.getAppointments(requireActivity()));
                        unfilteredMedicines.addAll(treatment.getMedicines(requireActivity()));
                    }

                    showHighlightedMedicalAppointments(unfilteredAppointments);
                    showHighlightedMedications(unfilteredMedicines);
                } catch (DBFindException exception) {
                    ExceptionManager.advertiseUI(requireActivity(), exception.getMessage());
                }
    ...
    public void onResume() {
        ...
        if (!treatments.isEmpty()) {
            ...
            try {
                ...
                for (Treatment treatment : treatments) {
                    appointments.addAll(treatment.getAppointments(requireActivity()));
                    medicines.addAll(treatment.getMedicines(requireActivity()));
                }

                showHighlightedMedicalAppointments(appointments);
                showHighlightedMedications(medicines);
            } catch (DBFindException exception) {
                ExceptionManager.advertiseUI(requireActivity(), exception.getMessage());
            }
    ...
    private void showHighlightedMedicalAppointments(List<MedicalAppointment> appointments) {
        ...
        for (MedicalAppointment appointment : appointments)
            if (!highlightedDates.contains(CalendarDay.from(appointment.getDateTime().getYear(), appointment.getDateTime().getMonthValue(), appointment.getDateTime().getDayOfMonth())))
    ...
    private void showSelectedDateSchedule() {
        ...
        boolean isSomethingScheduled = false;
        try {
            for (Treatment treatment : user.getTreatments(requireActivity())) {
                if (!treatment.getStartDate().toLocalDate().isAfter(selectedDate) && (treatment.getEndDate() == null || !treatment.getEndDate().toLocalDate().isBefore(selectedDate))) {
                    ...
                    if (hasStarted || hasFinished || !appointmentsToShow.isEmpty() || !medicinesToShow.isEmpty()) {
                        if (!isSomethingScheduled)
                            isSomethingScheduled = true;

                        LinearLayout linearLayoutTreatment = new LinearLayout(requireActivity());
                        linearLayoutTreatment.setLayoutParams(new LinearLayout.LayoutParams(
                                LinearLayout.LayoutParams.MATCH_PARENT,
                                LinearLayout.LayoutParams.WRAP_CONTENT
                        ));
                        linearLayoutTreatment.setOrientation(LinearLayout.VERTICAL);
                        ...
                        linearLayoutTreatment.addView(separatorView);

                        TextView textViewTreatment = new TextView(requireActivity());
                        textViewTreatment.setLayoutParams(new LinearLayout.LayoutParams(
                                LinearLayout.LayoutParams.MATCH_PARENT,
                                LinearLayout.LayoutParams.WRAP_CONTENT));
                        params = (LinearLayout.LayoutParams) textViewTreatment.getLayoutParams();
                        params.setMargins(0, getResources().getDimensionPixelSize(R.dimen.text_view_margin_top), 0, 0);
                        textViewTreatment.setLayoutParams(params);
                        value = new TypedValue();
                        requireActivity().getTheme().resolveAttribute(com.google.android.material.R.attr.textAppearanceTitleLarge, value, true);
                        textViewTreatment.setTextAppearance(value.data);
                        textViewTreatment.setText(treatment.getTitle());
                        linearLayoutTreatment.addView(textViewTreatment);

                        if (hasStarted)
                            setLimitSelectedDate(R.string.calendar_treatment_start, linearLayoutTreatment);
                        if (!appointmentsToShow.isEmpty())
                            setAppointmentsSelectedDate(appointmentsToShow, linearLayoutTreatment);
                        if (!medicinesToShow.isEmpty())
                            setMedicationsSelectedDate(medicinesToShow, linearLayoutTreatment);
                        if (hasFinished)
                            setLimitSelectedDate(R.string.calendar_treatment_end, linearLayoutTreatment);

                        linearLayout.addView(linearLayoutTreatment);
                    }
                }
            }
        } catch (DBFindException exception) {
    ...
    private void setLimitSelectedDate(int text, LinearLayout linearLayout) {
    ...
    private void setAppointmentsSelectedDate(List<MedicalAppointment> appointmentsToShow, LinearLayout linearLayout) {
    ...
    private void setMedicationsSelectedDate(List<Medicine> medicinesToShow, LinearLayout linearLayout) {
